      ...
      <div class="col-mid">
        <div class="card urg">
          <div class="card-hd">!! URGENT !! <span class="badge r">${suspend.length}</span>
            ...
            <button class="tbtn" style="font-size:11px;padding:3px 8px;border-color:#7a9bb5;color:#7a9bb5" onclick="openLogPanel()">📋 기록</button>
          </div>
...
const URGENT_FIELD_LABELS={
  ...
  urgPost2:'사후2차', urgUpload:'업로드 완료', urgCertAudit:'인증결정일 내 심사',
...
export function openLogPanel(){
  renderLogPanel();
  document.getElementById('log-panel').classList.add('on');
}
export function closeLogPanel(){document.getElementById('log-panel').classList.remove('on');}
...
export function renderLogPanel(){
  ...
  if(!logs.length){body.innerHTML='<div style="padding:20px;text-align:center;color:var(--muted);font-size:13px">아직 기록이 없어요</div>';return;}
  body.innerHTML=`<div style="max-height:60vh;overflow-y:auto">` + logs.map(l=>`
...
export function initApp() {
  ...
  if(window.FS){
    ...
    const keys=['emails2','checks','igcStore','overrides','edits','schedules','urgentLogs','fsscRaw','fsscName','jobRaw','jobName','igcName'];
    Promise.all(keys.map(async k=>{
      ...
      _startRender();
      // 실시간 동기화
      ['checks','overrides','igcStore','emails2','edits','schedules','urgentLogs'].forEach(key=>{
...
Object.assign(window, {addAlias, addEmail, addSchedule, buildNotice, calGoToday, calMove, calcMD, calcReset, calcSelectAdmin, calcUpdate, closeLogPanel, closeModal, copyCalcTotal, copyNoticePage, cycleUrgField, delEmail, delSchedule, editAlias, goSubTab, goTab, goTabById, loadEmailFile, noticeAutoComplete, noticePick, noticeSelect, openLogPanel, openModal, quickAddAlias, removeAlias, renderAll, renderCal, renderMultiList, saveReq, saveReqModal, saveReqReason, saveUpdDone, showCalPopup, toggleEditMode, toggleHidden, toggleTheme});
